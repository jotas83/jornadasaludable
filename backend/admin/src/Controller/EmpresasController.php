...
final class EmpresasController extends AdminController
{
    protected $text_prefix = 'COM_JORNADASALUDABLE_EMPRESA';

    public function getModel($name = 'Empresa', $prefix = 'Administrator', $config = ['ignore_request' => true])
    {
    ...
    private function setActivo(int $value): void
    {
        ...
        if ($ids) {
            ...
            $q = $db->getQuery(true)
                ->update($db->quoteName('#__js_empresas'))
                ->set($db->quoteName('activo') . ' = ' . (int) $value)
                ->whereIn($db->quoteName('id'), $ids);
            $db->setQuery($q)->execute();
            $msg = sprintf(Text::_('COM_JORNADASALUDABLE_N_ITEMS_GUARDADOS'), count($ids));
        }
        $this->setRedirect(Route::_('index.php?option=com_jornadasaludable&view=empresas', false), $msg);
    }
}
